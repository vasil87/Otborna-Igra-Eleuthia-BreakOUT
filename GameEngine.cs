namespace OtbornaIgra.GameEngines
{
...

    {
            ...
            //metoda koito e zaka4en za eventa presingkey
        {
            ...
            {
                var right = this.Pad.Position.Left;
                var top = this.Pad.Position.Top-15;
                var newPositon = new Position(right, top);
                if (this.renderer.isInBounds(newPositon))
                {
                    this.Pad.Position = newPositon;
        ...
        internal void InitGame()//////MMMMMMMMMMMMMMMMMM
        {
            Uri uri = new Uri(System.IO.Path.GetFullPath(@"..\..\Images\07 - New Rose.mp3"));
            MediaPlayer startSong = new MediaPlayer();
            startSong.Open(uri);
            startSong.Play();
        ...
        internal void StartGame()
        {
            ...
            timer.Tick += (sender, args) =>
            {
                ...
                if (CollisionDetector.CheckForCollisionWithBricks(this.Ball, this.Bricks))
                {
                   ...
                   //Console.Beep(65, 12);////
                    Uri uri = new Uri(System.IO.Path.GetFullPath(@"..\..\Images\zombi_pressure.wav"));
                    MediaPlayer startSong = new MediaPlayer();
                    startSong.Open(uri);
                    startSong.Play();
                }
